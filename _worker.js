...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            if (!userID) {
                return new Response('请设置你的有有ID变量，或尝试重试部署，检查变量是否生效？', {
                    status: 404,
                    headers: {
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (路径 == '/') {
                    ...
                    const 为了活Config = await 生成配置信息(userID, request.headers.get('Host'), sub, UA, RproxyIP, url, fakeUserID, fakeHostName, env);
                    const now = Date.now();
                ...
                return await 为了活OverWSHandler(request);
            }
        } catch (err) {
            let e = err;
...
async function 为了活OverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            const {
                ...
                addressRemote = '',
                rawDataIndex,
                为了活Version = new Uint8Array([0, 0]),
                isUDP,
            } = process为了活Header(chunk, userID);
            ...
            const 为了活ResponseHeader = new Uint8Array([为了活Version[0], 0]);

            const rawClientData = chunk.slice(rawDataIndex);

            if (isDns) {
                const { write } = await handleUDPOutBound(webSocket, 为了活ResponseHeader, log);
            ...
            if (!banHosts.includes(addressRemote)) {
                ...
                handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, 为了活ResponseHeader, log);
            } else {
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 为了活ResponseHeader, log,) {
    ...
    async function nat64() {
        ...
        remoteSocketToWS(tcpSocket, webSocket, 为了活ResponseHeader, null, log);
    }
    ...
    async function retry() {
        ...
        */
        ...
        remoteSocketToWS(tcpSocket, webSocket, 为了活ResponseHeader, nat64, log);
    }
    ...
    remoteSocketToWS(tcpSocket, webSocket, 为了活ResponseHeader, retry, log);
}
...
function process为了活Header(为了活Buffer, userID) {
    // 检查数据长度是否足够（至少需要 24 字节）
    if (为了活Buffer.byteLength < 24) {
        return {
            hasError: true,
    ...
    // 解析 为了活 协议版本（第一个字节）
    const version = new Uint8Array(为了活Buffer.slice(0, 1));
    ...
    // 使用函数验证
    isValidUser = isUserIDValid(userID, userIDLow, 为了活Buffer);

    // 如果用户 ID 无效，返回错误
    if (!isValidUser) {
        return {
            hasError: true,
            message: `invalid user ${(new Uint8Array(为了活Buffer.slice(1, 17)))}`,
        };
    }

    // 获取附加选项的长度（第 17 个字节）
    const optLength = new Uint8Array(为了活Buffer.slice(17, 18))[0];
    // 暂时跳过附加选项

    // 解析命令（紧跟在选项之后的 1 个字节）
    // 0x01: TCP, 0x02: UDP, 0x03: MUX（多路复用）
    const command = new Uint8Array(
        为了活Buffer.slice(18 + optLength, 18 + optLength + 1)
    )[0];
    ...
    const portBuffer = 为了活Buffer.slice(portIndex, portIndex + 2);
    ...
    const addressBuffer = new Uint8Array(
        为了活Buffer.slice(addressIndex, addressIndex + 1)
    );
    ...
    switch (addressType) {
        case 1:
            ...
            addressValue = new Uint8Array(
                为了活Buffer.slice(addressValueIndex, addressValueIndex + addressLength)
            ).join('.');
            break;
        case 2:
            // 域名
            ...
            addressLength = new Uint8Array(
                为了活Buffer.slice(addressValueIndex, addressValueIndex + 1)
            )[0];
            addressValueIndex += 1;
            // 解码域名
    ...
    return {
        ...
        为了活Version: version,	  // 为了活 协议版本
        isUDP,					 // 是否是 UDP 请求
    };
}

async function remoteSocketToWS(remoteSocket, webSocket, 为了活ResponseHeader, retry, log) {
    // 将数据从远程服务器转发到 WebSocket
    let remoteChunkCount = 0;
    let chunks = []; // @type {ArrayBuffer | null} 
    let 为了活Header = 为了活ResponseHeader;
        ...
        .pipeTo(
            new WritableStream({
                ...
                async write(chunk, controller) {
                    ...
                    if (为了活Header) {
                        // 如果有 为了活 响应头部，将其与第一个数据块一起发送
                        webSocket.send(await new Blob([为了活Header, chunk]).arrayBuffer());
                        为了活Header = null; // 清空头部，之后不再发送
                    } else {
                        // 直接发送数据块
...
async function handleUDPOutBound(webSocket, 为了活ResponseHeader, log) {

    let is为了活HeaderSent = false;
        ...
        async write(chunk) {
            ...
            if (webSocket.readyState === WS_READY_STATE_OPEN) {
                ...
                if (is为了活HeaderSent) {
                    ...
                    webSocket.send(await new Blob([为了活ResponseHeader, udpSizeBuffer, dnsQueryResult]).arrayBuffer());
                    is为了活HeaderSent = true;
                }
            }
        }
    })).catch((error) => {
...
async function handleDNSQuery(udpChunk, webSocket, 为了活ResponseHeader, log) {
    ...
    try {
        ...
        let 为了活Header = 为了活ResponseHeader; // 保存 为了活 响应头部，用于后续发送
        ...
        await tcpSocket.readable.pipeTo(new WritableStream({
            async write(chunk) {
                if (webSocket.readyState === WS_READY_STATE_OPEN) {
                    if (为了活Header) {
                        // 如果有 为了活 头部，则将其与 DNS 响应数据合并后发送
                        webSocket.send(await new Blob([为了活Header, chunk]).arrayBuffer());
                        为了活Header = null; // 头部只发送一次，之后置为 null
                    } else {
                        // 否则直接发送 DNS 响应数据
                        webSocket.send(chunk);
                    }
        ...
        if ((addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0) {
            ...
            if (hostName.includes("worker") || hostName.includes("notls")) {
                ...
                addressesnotls = addressesnotls.concat(
                    cfips.map(cidr => generateRandomIPFromCIDR(cidr) + ':' + randomPorts[Math.floor(Math.random() * randomPorts.length)] + '#CldFlr随机节点' + String(counter++).padStart(2, '0'))
                );
            } else {
            ...
            if (hostName.includes('workers.dev')) {
                if (proxyhostsURL && (!proxyhosts || proxyhosts.length == 0)) {
                    try {
                        ...
                        if (!response.ok) {
                            console.error('获取地址时错:', response.status, response.statusText);
                            return; 
                        }
    ...
    <script>
        ...
        async function loadConfig() {
            try {
                ...
                document.getElementById('loading').innerHTML = '<p style="color: red;">加载配置失败，请刷新重试</p>';
            }
        }
        ...
        function renderSubInfo() {
            ...
            let subItems = [
                { label: '名称', value: sub.SUBNAME },
                { label: '优选生成器', value: sub.SUB },
            ...
            if (sub.SUB === 'local') {
                subItems.push(
                    ...
                    { label: 'ADDNOTLS (NOTLS优选)', value: sub.ADDNOTLS.join('<br>') },
                    { label: 'ADDAPI (TLS API)', value: sub.ADDAPI.join('<br>') },
                    { label: 'ADDNOTLSAPI (NOTLS API)', value: sub.ADDNOTLSAPI.join('<br>') },
        ...
        function updateKVStatus() {
            ...
            if (configData.KV) {
                kvStatus.textContent = 'KV命名空间已绑定';
            } else {
                kvStatus.textContent = 'KV命名空间未绑定';
            }
        }
