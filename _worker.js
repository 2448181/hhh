    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        if (url.searchParams.has("proxyip")) {
          ...
          path = url.searchParams.has("globalproxy")
            ? `/?socks5=${url.searchParams.get("socks5")}&globalproxy`
            : `/?socks5=${url.searchParams.get("socks5")}`;
          RproxyIP = "false";
        } else if (url.searchParams.has("socks")) {
          path = url.searchParams.has("globalproxy")
            ? `/?socks5=${url.searchParams.get("socks")}&globalproxy`
            : `/?socks5=${url.searchParams.get("socks")}`;
          RproxyIP = "false";
        } else if (url.searchParams.has("http")) {
          path = url.searchParams.has("globalproxy")
            ? `/?http=${url.searchParams.get("http")}&globalproxy`
        ...
        if (路径 == "/") {
          ...
          (url.pathname == `/${动态UUID}/config.json` ||
            路径 == `/${userID}/config.json`) &&
          url.searchParams.get("token") === (await 双重哈希(fakeUserID + UA))
        ) {
          return await config_Json(
            userID,
            request.headers.get("Host"),
            sub,
            UA,
          ...
          url.pathname == `/${动态UUID}/edit` ||
          路径 == `/${userID}/edit`
        ) {
          return await KV(request, env);
        } else if (
          url.pathname == `/${动态UUID}/bestip` ||
          路径 == `/${userID}/bestip`
        ) {
        ...
        go2Socks5s = url.searchParams.has("globalproxy")
          ? ["all in"]
          : go2Socks5s;
...
async function handleTCPOutBound(
  ...
  async function retry() {
    if (enableSocks) {
      ...
      tcpSocket = await connectAndWrite(
        proxyIP.toLowerCase() || addressRemote,
        portRemote,
      );
    }
...
function 配置信息(UUID, 域名地址) {
  ...
  const 威图瑞 =
    ...
    "p" +
    `${atob("dGlvbj0=") + 加密方式}\u0026\u0073\u0065\u0063\u0075\u0072\u0069\u0074\u0079\u003d${传输层安全[0]}&sni=${SNI}&fp=${指纹}&type=${传输层协议}&host=${伪装域名}&path=${encodeURIComponent(路径) + allowInsecure}&fragment=${encodeURIComponent("1,40-60,30-50,tlshello")}#${encodeURIComponent(别名)}`;
    ...
    try {
      ...
      if ((!sub || sub == "") && isBase64 == true) {
        ...
        const response = await fetch(url, {
          headers: {
            "User-Agent": atob(
              "djJyYXlOL2VkZ2V0dW5uZWwgKGh0dHBzOi8vZ2l0aHViLmNvbS9jbWxpdS9lZGdldHVubmVsKQ==",
            ),
          },
        });
